...
# Blend factor for residual: 0 = raw only, 1 = full baseline subtraction
# 0.25-0.35 emphasizes diagnostic keywords while preserving smooth gradients
BLEND_ALPHA = 0.3


def _compute_baseline_saliency(raw_maps: List[np.ndarray]) -> np.ndarray:
    """Compute mean saliency across all word maps as baseline."""
    if not raw_maps:
        return None
...
def _apply_blended_residual(raw_map: np.ndarray, baseline: np.ndarray, alpha: float) -> np.ndarray:
    """
    Blend raw saliency with residual: (1-alpha)*raw + alpha*(raw - baseline).
    Preserves smooth gradients while emphasizing what's unique to this token.
    """
    raw = raw_map.astype(np.float32)
    base = baseline.astype(np.float32)
    residual = raw - base
    blended = (1.0 - alpha) * raw + alpha * residual
    blended_min, blended_max = blended.min(), blended.max()
    if blended_max - blended_min > 1e-10:
        blended = (blended - blended_min) / (blended_max - blended_min + 1e-10)
    else:
        blended = np.ones_like(blended, dtype=np.float32) * 0.5
...
def generate_word_level_saliency_and_pli(gradcam_instance, image: Image.Image, prompt: str, 
    ...
    # Phase 1: Collect raw saliency maps for each word
    raw_maps_by_word = {}
    valid_words = [w for w in words if w in word_to_tokens]
    ...
    for word_idx, word in enumerate(valid_words):
        ...
        try:
            ...
            tokens_before_word = caption_ids[:target_token_idx]
            full_input_ids = torch.cat(
                [input_ids, torch.tensor([tokens_before_word], device=device, dtype=torch.long)],
                dim=1
            )

            raw_saliency = gradcam_instance.generate_saliency_for_token(
                image, full_input_ids, target_token_id,
                original_size=(original_size[0], original_size[1])
            )
            if raw_saliency.max() > raw_saliency.min():
                raw_saliency = (raw_saliency - raw_saliency.min()) / (
                    raw_saliency.max() - raw_saliency.min() + 1e-10
                )
            raw_maps_by_word[word] = raw_saliency.astype(np.float32)

            if (word_idx + 1) % 5 == 0:
                print(f"    Processed {word_idx + 1}/{len(valid_words)} words...")

        except Exception as e:
            print(f"  ⚠️  Error generating saliency for word '{word}': {e}")

    if not raw_maps_by_word:
        print(f"  ⚠️  No raw saliency maps collected")
        return {}

    # Phase 2: Compute baseline, apply blended residual, build PLI
    baseline = _compute_baseline_saliency(list(raw_maps_by_word.values()))
    if baseline is None:
        return {}

    print(f"  📊 Applying blended residual (alpha={BLEND_ALPHA}) to emphasize diagnostic keywords...")

    word_saliency_pli_maps = {}
    for word, raw_map in raw_maps_by_word.items():
        saliency_map = _apply_blended_residual(raw_map, baseline, BLEND_ALPHA)
