...
class GradCAM:
    ...
    def _manual_generate(self, images: torch.Tensor, input_ids: torch.Tensor, max_new_tokens: int = 40,
        ...
        try:
            ...
            if vocab_size is not None:
                valid_token_ids = [tid for tid in new_token_ids if 0 <= tid < vocab_size]
            else:
                valid_token_ids = new_token_ids
        ...
        # Strip SentencePiece space marker if present
    ...
    def generate_saliency_for_token(self, image: Image.Image, input_ids: torch.Tensor, 
                                    target_token_id: int, original_size: Tuple[int, int]) -> np.ndarray:
        """
        Generate token-specific saliency using vision encoder Grad-CAM.
        Uses vision encoder activations/gradients (not image gradients) for meaningful spatial variation.
        """
        import torch as _torch
        device = next(self.model.parameters()).device
        ...
        processed = self.image_processor(image_resized, return_tensors="pt")
        image_tensor = processed["pixel_values"].to(device=device, dtype=self.model_dtype)
        ...
        if self.gradients is not None and self.activations is not None:
            grads = self.gradients.clone().detach()
            acts = self.activations.clone().detach()
            if grads.abs().sum().item() > 1e-6:
                if grads.dim() == 3:
                    grads, acts = grads[0], acts[0]
                elif grads.dim() == 4:
                    grads, acts = grads[0], acts[0]
                if grads.dim() == 2 and acts.dim() == 2:
                    expected_patches = 24 * 24
                    total_tokens = acts.shape[0]
                    if total_tokens == expected_patches + 1:
                        patch_grads, patch_acts = grads[1:, :], acts[1:, :]
                    elif total_tokens >= expected_patches:
                        patch_grads, patch_acts = grads[1:, :], acts[1:, :]
                    else:
                        patch_grads, patch_acts = grads, acts
                    weights = _torch.mean(patch_grads, dim=1)
                    features = _torch.mean(patch_acts, dim=1)
                    cam_1d = weights * features
                    num_patches = cam_1d.shape[0]
        ...
        if cam is None and self.activations is not None:
            acts = self.activations.clone().detach()
            if acts.dim() == 3:
                acts = acts[0]
            elif acts.dim() == 4:
        ...
        if cam is not None:
            ...
            if cam_max > cam_min + 1e-6:
                cam = (cam - cam_min) / (cam_max - cam_min + 1e-8)
                cam = _torch.pow(cam, 0.5)
                cam = (cam - cam.min()) / (cam.max() + 1e-8)
            cam_np = cam.detach().cpu().numpy().astype(np.float32)
        else:
            img_arr = np.array(image_resized)
            img_gray = np.mean(img_arr, axis=2) if len(img_arr.shape) == 3 else img_arr
            cam_np = (img_gray - img_gray.min()) / (img_gray.max() - img_gray.min() + 1e-8).astype(np.float32)
